...
unsigned Particion::h(unsigned ind_dato) const {
  //cout << "entered h, _Clusters.size="<<_clusters.size()<<"\n";
  int r = _clusters.at(ind_dato);
  //cout << "left h with r="<<r<<endl;
  return r;
}
...
bool Particion::esValida() const {
  //cout << "entered esvalida , w/ n_c=" << _n_clusters << endl;
  for ( unsigned i = 0; i < _n_clusters; i++ ) {
...
bool PAR::v(const Particion &part, unsigned i, unsigned j) const {
  //cout << "entered v : i=" << i <<", j="<<j << endl;
  if ( i == j ) {
    return false;
  }
  //cout << "again v with i="<<i;
  //cout<<" (h(i)="<<part.h(i);
  //cout<<"), j="<<j;
  //cout<<" (h(j)="<<part.h(j);
  //cout<<") -> restr=" << _restr_mat[i][j]<<endl;
...
Vecino *PAR::nextVecinoVirtual(const Particion &part) {
  cout << "Entered nextVecinoVirtual ---------------------" << endl;
  ...
  while (!candidata->esValida()) {
    cout << "    vecino inválido (next: "<<_expl_vecino_next<<", size="<<_orden_expl_vecinos.size()<<"): " << _orden_expl_vecinos[_expl_vecino_next].first <<" " << _orden_expl_vecinos[_expl_vecino_next].second << endl;
    if (_expl_vecino_next < _orden_expl_vecinos.size()) {
      Particion c = part.apply(_orden_expl_vecinos[_expl_vecino_next++]);
      candidata = &c;
    } else {
      cout << "    no hay más vecinos que explorar" << endl;
      return NULL;
    }
  }

  cout << "    vecino válido (next: "<<_expl_vecino_next<<", size="<<_orden_expl_vecinos.size()<<"): " << _orden_expl_vecinos[_expl_vecino_next].first <<" " << _orden_expl_vecinos[_expl_vecino_next].second << endl;
  cout << "-------------------------------"<<endl;
  return &(_orden_expl_vecinos[_expl_vecino_next++]);
}
